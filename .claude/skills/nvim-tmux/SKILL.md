...
- The `nvim` on `PATH` must be the real binary, OR `NVIM_TMUX_NVIM_BIN` must point at it. The plugin handles this automatically (`vim.v.progpath`); only override by exporting it before nvim starts.
...
State holds a stale `nvim_winid` (e.g. the user `:close`d a pane's window by hand). `send-keys` self-heals — `open_terminal` checks `win_id2win` and materializes a fresh split, rebinding state — but `select-pane`/`resize-pane` on a stale id error out. The leader pane's winid is cached on first use by `leader_winid()` (via `vim.fn.win_getid()`). State lives in `vim.g.nvim_tmux` inside the running nvim — restart nvim to clear it, or `:lua vim.g.nvim_tmux = nil` from inside that nvim.
...
### `nvim-tmux: nvim RPC failed (is the nvim behind $NVIM still alive?)`
`$NVIM` is set but the socket is dead — the nvim that exported it exited (or the env leaked into a new shell). Deliberately a hard error rather than a silent "no": a boolean query like `has-session` must not read a transport failure as "session doesn't exist". Re-launch from a `:terminal` of a live nvim.
