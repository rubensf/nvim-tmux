...
> Fittingly, this was *carefully* vibe coded :) — written with Claude Code, then audited, minimized, and adversarially reviewed with it too. The test suite drives a real headless nvim through Claude's exact audited tmux call sequences, so the vibes are load-bearing **and** verified.
...
The repo ships a project skill ([`.claude/skills/nvim-tmux/SKILL.md`](.claude/skills/nvim-tmux/SKILL.md)) — a setup + architecture + debug runbook that Claude Code picks up automatically when you work in this repo. Run `claude` here and ask about a failure (or invoke it explicitly with `/nvim-tmux`), and Claude knows the state-dump probes, the failure patterns above, and where each subcommand's code path lives. It covers the install flow too, so "help me set this up" works cold.
...
```mermaid
flowchart TD
    plug["plugin/nvim-tmux.lua — auto-load stub"]
    claude["claude (Agent Teams) — in an nvim :terminal"]
    tmux["bin/tmux — parse argv, route the subcommand"]
    rpc["nvim_expr — nvim --server $NVIM --remote-expr"]
    mod["lua/nvim-tmux/init.lua — state + actions + setup()"]
    nvim["nvim windows · buffers · terminal jobs · vim.g.nvim_tmux"]

    plug -. "setup() puts bin/ on PATH" .-> claude
    claude -- "tmux split-window …" --> tmux
    tmux -- "one RPC per subcommand" --> rpc
    rpc -- "v:lua.require'nvim-tmux'.X(…)" --> mod
    mod -- "vim.cmd / vim.api" --> nvim
```
...
For the full picture — layer-by-layer breakdown, the state shape, and a complete data-flow trace — see the **[rendered architecture overview](https://rubensf.github.io/nvim-tmux/architecture.html)**.
...
Cross-module contracts (state schema, send-keys grammar, transport helpers, exit conventions) live in [`docs/CONTRACTS.md`](docs/CONTRACTS.md). A richer visual overview (state shape, install paths, full data-flow trace) is rendered at **[rubensf.github.io/nvim-tmux/architecture.html](https://rubensf.github.io/nvim-tmux/architecture.html)** (source: `docs/architecture.html`).
